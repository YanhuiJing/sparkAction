package com.spark.electricStore.spark.ad;
...
import com.spark.electricStore.conf.ConfigurationManager;
import com.spark.electricStore.constant.Constants;
import com.spark.electricStore.dao.*;
import com.spark.electricStore.dao.factory.DAOFactory;
import com.spark.electricStore.domain.*;
import com.spark.electricStore.util.DateUtils;
import com.spark.electricStore.dao.*;
import com.spark.electricStore.domain.*;
...
//				.set("spark.streaming.receiver.writeAheadLog.enable", "true");
		...
		List<String> topics = new ArrayList<>(0);
...
//		adRealTimeLogDStream.repartition(1000);
		...
		// 根据动态黑名单进行数据过滤,使用transform算子,实时获取新的rdd名单通过join过滤
...
//		calculateAdClickCountByWindow(adRealTimeLogDStream);
		...
		JavaPairDStream<String, String> filteredAdRealTimeLogDStream = adRealTimeLogDStream.transformToPair(
				...
				new Function<JavaRDD<ConsumerRecord<String, String>>, JavaPairRDD<String,String>>() {
					...
					public JavaPairRDD<String, String> call(
							JavaRDD<ConsumerRecord<String, String>> rdd) throws Exception {
                        ...
                        new PairFunction<ConsumerRecord<String, String>, Long, Tuple2<String, String>>() {
							...
							public Tuple2<Long, Tuple2<String, String>> call(
								...
								return new Tuple2<Long, Tuple2<String, String>>(userid, new Tuple2(tuple.key(),tuple.value()));
							}
                        ...
                        JavaPairRDD<Long, Tuple2<Tuple2<String, String>, org.apache.spark.api.java.Optional<Boolean>>> joinedRDD = mappedRDD.leftOuterJoin(blacklistRDD);

                        JavaPairRDD<Long, Tuple2<Tuple2<String, String>, org.apache.spark.api.java.Optional<Boolean>>> filteredRDD = joinedRDD.filter(new Function<Tuple2<Long, Tuple2<Tuple2<String, String>, org.apache.spark.api.java.Optional<Boolean>>>, Boolean>() {
                            @Override
                            public Boolean call(Tuple2<Long, Tuple2<Tuple2<String, String>, org.apache.spark.api.java.Optional<Boolean>>> tuple) throws Exception {
                                org.apache.spark.api.java.Optional<Boolean> optional = tuple._2._2;
						...
						JavaPairRDD<String, String> resultRDD = filteredRDD.mapToPair(
								...
								new PairFunction<Tuple2<Long,Tuple2<Tuple2<String,String>,org.apache.spark.api.java.Optional<Boolean>>>, String, String>() {

									private static final long serialVersionUID = 1L;

									@Override
									public Tuple2<String, String> call(
											Tuple2<Long, Tuple2<Tuple2<String, String>, org.apache.spark.api.java.Optional<Boolean>>> tuple)
											throws Exception {
										return tuple._2._1;
									}
	...
	private static JavaPairDStream<String, Long> calculateRealTimeStat(
		...
		JavaPairDStream<String, Long> aggregatedDStream = mappedDStream.updateStateByKey(
				...
				new Function2<List<Long>, org.apache.spark.api.java.Optional<Long>, org.apache.spark.api.java.Optional<Long>>() {

					private static final long serialVersionUID = 1L;

					@Override
					public org.apache.spark.api.java.Optional<Long> call(List<Long> values, org.apache.spark.api.java.Optional<Long> optional)
							throws Exception {
						// 举例来说
						// 对于每个key，都会调用一次这个方法
						...
						return org.apache.spark.api.java.Optional.of(clickCount);
					}
		...
		JavaDStream<Row> rowsDStream = adRealTimeStatDStream.transform(
				...
				new Function<JavaPairRDD<String,Long>, JavaRDD<Row>>() {
					...
					public JavaRDD<Row> call(JavaPairRDD<String, Long> rdd)
						...
						Dataset provinceTop3AdDF = sqlContext.sql(
								"SELECT "
									+ "date,"
									+ "province,"
									+ "ad_id,"
