package com.spark.electricStore.spark.product;
...
import com.spark.electricStore.domain.AreaTop3Product;
import com.spark.electricStore.util.ParamUtils;
import com.spark.electricStore.util.SparkUtils;
...
//		JSONObject taskParam = JSONObject.parseObject(task.getTaskParam());
		String testParam = "{\"startAge\":[\"10\"],\"endAge\":[\"50\"],\"startDate\":[\"2020-03-20\"],\"endDate\":[\"2020-03-20\"]}";
		JSONObject taskParam = JSONObject.parseObject(testParam);
		...
		// 从MySQL中查询城市信息
		// 技术点2：异构数据源之MySQL的使用
		JavaPairRDD<Long, Row> cityid2cityInfoRDD = getcityid2CityInfoRDD(sc);
	 ...
	 * @return
	 */
	private static JavaPairRDD<Long, Row> getcityid2CityInfoRDD(JavaSparkContext javaSparkContext) {
...
//		return cityid2cityInfoRDD;
		...
		List<Tuple2<Long,Row>> cityList= Arrays.asList(
				new Tuple2<Long, Row>(0L,RowFactory.create(0L,"北京","华北")),
				new Tuple2<Long, Row>(0L,RowFactory.create(1L,"天津","华北")),
				new Tuple2<Long, Row>(0L,RowFactory.create(2L,"石家庄","华北")),
				new Tuple2<Long, Row>(0L,RowFactory.create(3L,"上海","华东")),
				new Tuple2<Long, Row>(0L,RowFactory.create(4L,"杭州","华东")),
				new Tuple2<Long, Row>(0L,RowFactory.create(5L,"南京","华东")),
				new Tuple2<Long, Row>(0L,RowFactory.create(6L,"郑州","华中")),
				new Tuple2<Long, Row>(0L,RowFactory.create(7L,"武汉","华中")),
				new Tuple2<Long, Row>(0L,RowFactory.create(8L,"广州","华南")),
				new Tuple2<Long, Row>(0L,RowFactory.create(9L,"深圳","华南"))
		);

		JavaPairRDD<Long, Row> cityid2cityInfoRDD = javaSparkContext.parallelize(cityList).
					mapToPair(tuple -> new Tuple2<Long, Row>(tuple._1, tuple._2));
...
//		String sql = "select * from tmp_area_fullprod_click_count  limit 10";
